...
unsigned gen_fac(unsigned number_index, int fac_num) {
	...
	code("\n\t\tCMPS \t");
        ...
        gen_sym_name(i);
        code(",$0");
        code("\n\t\tJLEU \t@for_fac_end%d", fac_num);
        
        // else: res *= i
        code("\n@for_fac_body%d:", fac_num);
        code("\n\t\tMULU\t");
        gen_sym_name(res);
        code(",");
        gen_sym_name(i);
        ...
        code(",$1,");
        gen_sym_name(i);
        code("\n\t\tJMP \t@for_fac_test%d", fac_num);
        
        code("\n@for_fac_end%d:", fac_num);
...
unsigned gen_exp(unsigned number_index, unsigned exp_index, int exponent_num) {
	...
	set_type(res, UINT);
        ...
        gen_sym_name(i);
        code(",$0");
        code("\n\t\tJLEU \t@for_exp_end%d", exponent_num);
        
        // else: res *= value(number_index)
        code("\n@for_exp_body%d:", exponent_num);
        code("\n\t\tMULU\t");
        gen_sym_name(res);
        code(",");
        gen_sym_name(number_index);
        ...
        code(",$1,");
        gen_sym_name(i);
        code("\n\t\tJMP \t@for_exp_test%d", exponent_num);
        
        code("\n@for_exp_end%d:", exponent_num);
